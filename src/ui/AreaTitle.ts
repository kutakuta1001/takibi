...
/**
 * スポット到着時（開始直後・遷移完了時）に画面上部中央へ場所名を表示するタイトルカード。
 * 次の一歩の案内は StoryPanel の本文が担うため、hint は省略可能（既定は空文字）。
...
export class AreaTitle {
  ...
  /** 場所名（+ 省略可能な補足）を表示し、ホールド後にゆっくりフェードアウトする。再呼び出しでタイマーを立て直す。 */
  show(name: string, hint = ''): void {
